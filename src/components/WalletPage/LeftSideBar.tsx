...
const LeftSideBar = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-between w-full space-y-2 text-bold h-full py-5">
      ...
      <div className="w-full border-t dark:border-white/20 px-4 flex flex-col items-center gap-3 pt-5">
        {SecondaryOption.map((option, index) => (
          <button
            key={index}
            className={cn(
              'flex items-center gap-2 w-full space-x-3 sm:p-3 text-black dark:text-white hover:bg-black hover:text-white text-nowrap rounded-lg',
