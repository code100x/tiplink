...
export default function SendToken({ setCurrent }: SendTokenProps) {
  ...
  return (
    <>
        ...
        <div className="w-full max-w-2xl mx-auto  space-y-6">
          ...
          <Card>
            <CardContent className="p-0">
              <Button
                variant="ghost"
                className="w-full justify-between py-8 text-left hover:bg-blue-400/10"
                onClick={() => {
                  setTypeOfSend('type1')
                }}
              >
              ...
              <Button
                variant="ghost"
                className="w-full justify-between py-8 text-left border-t hover:bg-blue-400/10"
      ...
      {typeOfSend === 'type1' && (
        <LinkTransfer setType={() => setTypeOfSend('')} />
      )}

      {typeOfSend === 'type2' && (
        <WalletTransfer setType={() => setTypeOfSend('')} />
      )}
    </>
  )
}
